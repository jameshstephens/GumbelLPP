...
\begin{theorem}[Approximate Gumbel Convergence]
  \label{thm:main}
  Assume the properties in Definitions \ref{def:exact_gumbel_convergence}, \ref{def:time_constant_gumbel}, and \ref{def:time_constant_exp}. Let $\alpha > 2/3$ and $N_n = \lfloor n^\alpha \rfloor$. For a sequence of Gumbel grids $Y^{(n)}$ and any $r \in \mathbb{R}$:
  \[
...
\begin{proof}
  \uses{thm:coupling_upper_bound, thm:coupling_lower_bound, thm:slutsky_cdf, lem:converges_in_prob_mul_zero, lem:deterministic_factor_limit}

  By Theorems \ref{thm:coupling_upper_bound} and \ref{thm:coupling_lower_bound}:
  \[
    0 \leq T_{\text{Approx}}^{N_n}(n) - T_{\text{Gumbel}}(n) \leq \frac{1}{N_n} L_{\text{Exp}}(n)
  \]

  Dividing by $\sigma_g n^{1/3}$:
  \[
    \left|\frac{T_{\text{Approx}}^{N_n}(n) - T_{\text{Gumbel}}(n)}{\sigma_g n^{1/3}}\right| \leq \frac{L_{\text{Exp}}(n)}{N_n \sigma_g n^{1/3}}
  \]

  By Definition \ref{def:time_constant_exp}, $L_{\text{Exp}}(n)/n \xrightarrow{\mathbb{P}} D_L$. By Lemma \ref{lem:deterministic_factor_limit}, $n/(N_n n^{1/3}) \to 0$. Therefore by Lemma \ref{lem:converges_in_prob_mul_zero}:
  \[
  ...
  Applying Slutsky's Theorem (Theorem \ref{thm:slutsky_cdf}) completes the proof.
\end{proof}
